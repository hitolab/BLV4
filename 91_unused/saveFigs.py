#二つのグラフの画像を一枚に出力するプログラム
#改善点：二つじゃなくて複数のグラフを一括して表示したい。関数もそれに従いたい。
...
figDataPath1 = "/Users/sunao/python_coding/BLV4/results/1/0035_1.txt"
figDataPath2 = "/Users/sunao/python_coding/BLV4/results/1/0035_1.txt"
...
def makeFig():
    # データの取得
    time1, data1 = _getData(figDataPath1)
    time2, data2 = _getData(figDataPath2)

    # 横軸表示日数
    # 少数点以下切り上げ。最低でも3以上になるように
    display_days = [int(max(np.ceil(max(time1)/24), 3)),
                    int(max(np.ceil(max(time2)), 3))]
