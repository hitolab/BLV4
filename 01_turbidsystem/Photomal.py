...
class Photomal:
    ...
    def __init__(self, no):
        print("no=", +no)        # "{}".format（）:手前の{}の中に変数()をと置換する。
        fileno = len(os.listdir(path="./data/results/{}".format(no)))+1
        self.no = no
        self.filepath = "./data/results/{}/{:04}_{}.txt".format(
            self.no, fileno, self.no)
    ...
    def measure(self):
        ...
        print("PMT{}:".format(self.no) +
              dt_now.strftime('%Y-%m-%d-%H-%M-%S ')+"measured "+str(self.val))
        # ここまで定期実行
    ...
    def __command(self, char, n=""):
        if n == "":
            ...
            ret = char.encode('utf-8') + n.to_bytes(2,
                                                    byteorder="big")+"\r".encode('utf-8')
        else:
            ret = char.encode('utf-8') + n.to_bytes(1,
                                                    byteorder="big")+"\r".encode('utf-8')
        return ret
    ...
    def __initialize_communication(self):
        ...
        self.ser = serial.Serial(
            comport, baudrate=9600, parity=serial.PARITY_NONE)
